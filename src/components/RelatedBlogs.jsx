...
const ReleatedBlogSection = ({ current }) => {
  ...
  // Exclude the current blog by filtering out its slug
  const filteredBlogs = data.filter((item) => item["slug"] !== current);

  // Get the latest 3 blogs excluding the current one
    ...
    <>
      <div className="min-h-screen bg-gray-50 py-8">
        <div className="container mx-auto px-4">
          <h2 className="text-3xl lg:text-3xl font-[500] text-teal-600 mb-7">
            Latest Blogs
          </h2>
          {error ? (
...
export default ReleatedBlogSection;
